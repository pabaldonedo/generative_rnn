...
class HiddenRNN(RNN):
    ...
    def __init__(self, n_in, n_hidden, activation, bias_init, prng):
        ...
        :param bias_init: list with bias initialization for [layers biases] + [output bias].
        ...
        "Activation list must have len(n_hidden). Activation: {0!r}, n_hidden: \
                                                {1!r}".format(self.activation_list, self.n_hidden)
        ...
        "Bias initialization list must have len(n_hidden). Bias list: {0!r}, n_hidden: \
                                                        {1!r}".format(self.bias_init, self.n_hidden)
...
class VanillaRNN(HiddenRNN):
    ...
    def initialize_weights(self):
        self.x = T.tensor3(name='x')
                #Number of weights and biases of hidden cells
        hidden_shape = np.sum(self.n_hidden ** 2) + self.n_in * self.n_hidden[0] + \
                    np.sum(self.n_hidden[:-1]*self.n_hidden[1:]) + \
                    np.sum(self.n_hidden) + np.sum(self.n_hidden)
        theta_shape = hidden_shape + self.n_hidden[-1]*self.n_out + self.n_out
        #Weight and biases variable
        self.theta = theano.shared(value=np.zeros(theta_shape, dtype=theano.config.floatX))
        ...
        param_idx, Wr_init = self.initialize_recursive_weights(param_idx)
        param_idx, W_forward_init = self.initialize_forward_weights(param_idx)
        param_idx, W_out_init = self.initialize_output_weights(param_idx)
        param_idx, h0_init = self.initialize_hidden_units(param_idx)
        param_idx, bh_init = self.initialize_bias(param_idx)
        param_idx, by_init = self.initialize_out_bias(param_idx)
        assert(param_idx == self.theta.shape.eval())

        self.pack_weights()
        self.theta.set_value(np.concatenate([x.ravel() for x in
            (Wr_init, W_forward_init, W_out_init, h0_init, bh_init, by_init)]))
        self.define_network()
    ...
    def initialize_out_bias(self, param_idx):
        ...
        self.by.name = 'by'
        by_init = self.bias_init[-1]*np.ones((self.n_out,), dtype=theano.config.floatX)
        param_idx += self.n_out
    ...
    def initialize_output_weights(self, param_idx):
        ...
        W_out_init = np.ones((self.n_hidden[-1], self.n_out), dtype=theano.config.floatX)

                    #np.asarray(self.prng.uniform(size=(self.n_hidden[-1], self.n_out),
                     #                             low=-0.01, high=0.01),
                     #                             dtype=theano.config.floatX)
        param_idx += self.n_hidden[-1] * self.n_out
    ...
    def define_network(self):
        ...
        def step(x_t, h_tm1):
            ...
            for i, h_units in enumerate(self.n_hidden):
                ...
                idx += h_units
            y_t = self.activation[-1](T.dot(h_t[:,(idx-self.n_hidden[-1]):idx], self.W_out) + self.by)
            return h_t, y_t
        ...
        [self.h, self.y_t], _ = theano.scan(step,
                    sequences=self.x,
                    outputs_info=[T.alloc(self.h0_as_vector, self.x.shape[1],
        ...
        self.predict = theano.function(inputs=[self.x], outputs=self.y_t)
        self.defined = True

    def generative_network(self):


        sample_steps = T.iscalar('n_steps')
        def step(h_tm1, y_tm1):
            ...
            for i, h_units in enumerate(self.n_hidden):
                if i == 0:
                    ha_t = T.set_subtensor(ha_t[idx:(idx+h_units)], T.dot(h_tm1[idx:(idx + h_units)], self.Wr[i]) + T.dot(y_tm1, self.W_forward[i]) + self.bh[i])
                    h_t = T.set_subtensor(h_t[idx:(idx+h_units)], self.activation[i](ha_t[idx:(idx+h_units)]))
                else:
                    ha_t = T.set_subtensor(ha_t[idx:(idx+h_units)], T.dot(h_tm1[idx:(idx + h_units)], self.Wr[i]) + T.dot(h_t[(idx-self.n_hidden[i-1]):idx], self.W_forward[i]) + self.bh[i])
                    h_t = T.set_subtensor(h_t[idx:(idx+h_units)], self.activation[i](ha_t[idx:(idx+h_units)]))
        ...
        [self.h_sample, self.y_sample], _ = theano.scan(step,
                    outputs_info=[self.h[-1,0,:], self.y_t[-1,0,:]], n_steps=sample_steps)

        
        self.sample = theano.function(inputs=[self.x, sample_steps], outputs=self.y_sample)